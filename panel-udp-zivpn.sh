...
mostrar_usuarios() {
    ...
    echo "Usuario|Expira|Estado" > /tmp/.zivpn.tmp
    local now=$(date +%s)
    while IFS='|' read -r usuario exp; do
        exp_ts=$(date -d "$exp" +%s 2>/dev/null)
        estado=$([ "$exp_ts" -gt "$now" ] && echo "Activo" || echo "Vencido")
        echo "$usuario|$exp|$estado" >> /tmp/.zivpn.tmp
    done < "$DB"
    column -t -s '|' /tmp/.zivpn.tmp
    echo
}
...
╚═══════════════════════════════════════════════════════╝${RESET}"
    ...
    # actualizar estado visual después de cada vuelta
    estado_clean=$(cat "$AUTO_CLEAN_FILE")
    estado_text=$([ "$estado_clean" == "ON" ] && echo "${GREEN}[ON]${RESET}" || echo "${RED}[OFF]${RESET}")
done
